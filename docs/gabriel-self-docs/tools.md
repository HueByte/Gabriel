...
Inside the Docker image the docs ship pre-baked at `/app/docs/` via a BuildKit named context (see `docker/docker-compose.yml`'s `additional_contexts.docs` and the runtime-stage `COPY --from=docs` step in `docker/Dockerfile.api`) — so the walk from `AppContext.BaseDirectory = /app/` resolves `docs/gabriel-self-docs` on the first probe with no host volume required. The compose file also mounts the host `docs/` at `/app/docs:ro` for dev so edits on the host show up without rebuilding the image. Local `dotnet run` still works the same way — the walk-up finds the docs at the repo root.
